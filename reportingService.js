...
var exactiulog = function() {
  if(DEBUG_MODE) {
    console.log.apply(console, arguments);
  }
  // console.assert(!DEBUG_MODE, logAttrs, args);
}
var clientSettings = {
  eventPrefix: '[EX-PKC]', // Prefix all events.
  dlPrefix: 'DLV-',
  cookieName: 'EX.PKC.context',  // Namespace the entire cookie usage to prevent collisions. Can be used across the entire domain.
  expireAfterDays: 90, // Number of days the cookie and localStorage will be considered fresh to consume.
  autoFillPrefix: "UB_AF_",
  propagation: true, // should we propagate to fb, hj, etc. events directly or rely on GTM
  propagateTo: ['fb', 'hj'], // fb - facebook, hj - hotjar, int - intercom
  fbPixelId: productionMode ? '380548409235401' : '876010999449346',  // facebook pixel id. multi-pixels aren't supported in this version
  hjid: productionMode ? 1330362 : 1335745, // hotjar site id
  notificationsCookieName: 'CookieNotificationCookie', // GDPR/"allow cookies" cookie name. Default is 'CookieNotificationCookie'.
  // Mark as bots or test users: these IPs will be flagged with "IP_flag"
  ipBlackList: ['174.106.141.177', '73.79.229.12', '73.216.88.123', '75.106.127.125', '148.75.225.65', '68.4.26.233', '69.73.57.126', '71.227.140.221','47.134.136.248','108.225.11.179', '174.57.174.26', '103.244.142.163'],
  // These params will be chained to every event, if available
  contextParamsKeys: ['uid', 'IP', 'variant', 'country', 'region', 'city', 'IP_flag',
                      'utm_source', 'utm_content', 'utm_campaign', 'utm_term', 'utm_medium',
                      'utm_campaignid', 'utm_adgroupid', 'utm_adid', 'utm_keyword',
                      'utm_matchtype', 'gclid', 'dimension1'],
  ga_dimensions: {
    variant: productionMode ? 1 : 6
  },
  events: {
    page_load: { // on load of the page
      defaults: {
        category: 'interest',
        action: 'page_load',
        nonInteraction: true
      ...
      propagate: [{fb: 'page_load'}, {hj: 'page_load'}],
      // choose either attributes or set your own
      setUserDimensions: [
        ['action', 'label'] // example: ["page_load", "variantE-cat"]
      ]
    },
    wizard_answer: { // wizard event click
      defaults: {
        category: 'wizard',
        action: 'wizard_answer' // better to replace with question name
        // label: '',
        // value: ''
      },
      propagate: [{fb: 'wizard_answer'}, {hj: 'wizard_answer'}],
      // choose either attributes or set your own
      setUserDimensions: [
        ['action', 'label'] // example: ["pre_existing_conditions", "No"]
      ]
    },
    main_cta_click: {
      defaults: {
        category: 'intent',
        action: 'main CTA click'
      },
      propagate: [{fb: 'CustomizeProduct'}, {hj: 'main_cta_click'}]
    },
    cta_click: {
      defaults: {
        category: 'interest',
      ...
      propagate: [{fb: 'cta_click'}, {hj: 'cta_click'}]
    },
    change_species: {
      defaults: {
        category: 'interest',
    ...
    read_more_click: {
      ...
      propagate: [{fb: 'Lead'}, {hj: 'ContactCreated'}]
    },
    video_play: {
      defaults: {
        category: 'interest',
        action: 'Video Play Click'
      },
    },
    toggle_click: {
      defaults: {
    ...
    quote_generate: {
      ...
      propagate: [{fb: 'CompleteRegistration'}, {hj: 'CompleteRegistration'}]
    },
    zip_code_entry: {
      defaults: {
        category:'intent',
        action: 'Zip Code Entry'
      },
    },
    deductibles_change: {
      defaults: {
        category:'interest',
        action: 'Deductible Change'
      },
    }
  }
};
